...
const ProductFamilyClient = ({ variants, product }) => {
  const [selectedOptions, setSelectedOptions] = useState({ v1: "", v2: "", v3: "" });
  const [quantity, setQuantity] = useState("1");
  const [isAdding, setIsAdding] = useState(false);
  const [activeImg, setActiveImg] = useState(product.productImg1 || null);
  const { addToCart } = useCart();

  const images = [product.productImg1, product.productImg2, product.productImg3].filter(
              ...
              {images.map((img, i) => (
                <button
                  ...
                  className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${
                    activeImg === img ? "border-blue-500" : "border-gray-200"
                  }`}
                >
